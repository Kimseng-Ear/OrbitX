import { Satellite, Radio, Clock, Wifi, Moon, Sun } from 'lucide-react'
...
export default function TopBar({ issData, isLoading, isNightMode, setNightMode }) {
  const now = new Date()
  const utc = now.toUTCString().split(' ').slice(1, 5).join(' ')

  return (
    <div className="glass border-b border-neon-blue/20 px-6 py-3 flex items-center justify-between"
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          <div className="text-sm font-bold tracking-widest gradient-text">EARTH FROM SPACE</div>
          <div className="text-[9px] text-neon-blue/50 font-mono tracking-[0.2em]">LIVE DASHBOARD v2.1</div>
        </div>
      </div>
      ...
      <div className="flex items-center gap-6">
        <div className="flex items-center gap-3">
          ...
          >
            ...
            <span className="text-[10px] font-bold tracking-wider uppercase">
              {isNightMode ? 'NIGHT OPS' : 'DAYLIGHT'}
            </span>
          </button>
        </div>
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="text-[10px] font-mono text-neon-blue/60">SIGNAL STRONG</div>
